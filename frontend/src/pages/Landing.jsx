...
const ArrowIcon = () => (
  <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
    <path
      d="M1 6.5H11M7 2.5L11 6.5L7 10.5"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label className="font-body text-xs font-semibold text-gray-700 uppercase tracking-wider">
          {label}
        </label>
        {hint && (
          <span className="font-body text-xs text-gray-400">{hint}</span>
        )}
      </div>
      <div
        className={[
...
function NameModal({ intent, onClose, onCreateSuccess }) {
  ...
  const submit = () => {
    ...
    if (isJoin) {
      ...
      socket.emit("join-room", { roomId, username: name.trim() });
        ...
        setTimeout(
          () =>
            navigate(`/game/${roomId}`, {
              state: { username: name.trim(), roomId },
            }),
          200,
        );  
      });
        ...
        <div className="p-8">
          ...
          <p className="font-body text-xs text-gray-400 text-center mt-4">
            {isJoin
              ...
              : "Saved locally, never on our servers."}
          </p>
        </div>
      </div>
    </div>
    ...
    <section className="relative min-h-screen flex flex-col justify-between pt-16 overflow-hidden">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 w-full flex flex-col justify-center flex-1 py-20">
        <div className="max-w-3xl">
          <div
            className="inline-flex items-center gap-2.5 bg-white/90 border border-black/10 rounded-full px-4 py-2 mb-10 shadow-sm"
          ...
          <p
            className="font-body text-lg text-gray-500 leading-relaxed max-w-lg mb-12"
            style={{ animation: "fadeUp 0.55s 0.14s ease both" }}>
            A shared 2D world where proximity drives conversation. Walk up to
            someone and you're talking. Walk away and you're not.
          </p>
          ...
          <div
            className="grid grid-cols-2 md:grid-cols-4 gap-8 pt-10 border-t border-black/5"
            style={{ animation: "fadeUp 0.55s 0.28s ease both" }}>
            {STATS.map(({ value, label }) => (
              <div key={label}>
                <p className="font-display font-extrabold text-gray-950 text-2xl tracking-tight mb-1.5">
                  {value}
                </p>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 w-full pb-8 flex items-center gap-3">
        <div className="w-px h-9 bg-linear-to-b from-gray-300 to-transparent" />
  ...
  return (
    <div className="flex overflow-hidden">
      <div
        ...
        {doubled.map((item, i) => (
          <div key={i} className="flex items-center gap-5 px-10 group">
            {/* Dot */}
            <span className="w-2 h-2 rounded-full bg-indigo-400/60 shrink-0 group-hover:bg-indigo-300 transition-colors duration-300" />

            {/* Text */}
            <span className="font-body text-base md:text-lg font-semibold text-indigo-200/80 whitespace-nowrap tracking-wide group-hover:text-white transition-colors duration-300">
              {item}
            </span>
          </div>
        ))}
...
function MarqueeBar() {
  return (
    <div
      className="relative overflow-hidden border-y border-white/10 select-none"
      style={{
        background:
          "linear-gradient(135deg,#1e1b4b 0%,#2d2a6e 50%,#1e1b4b 100%)",
      }}>
      {/* Side gradient fades */}
      <div className="absolute inset-y-0 left-0 w-48 bg-gradient-to-r from-[#1e1b4b] to-transparent z-10 pointer-events-none" />
      <div className="absolute inset-y-0 right-0 w-48 bg-gradient-to-l from-[#1e1b4b] to-transparent z-10 pointer-events-none" />
      ...
      <div className="py-10 space-y-6">
        <MarqueeTrack items={ROW_1} speed={46} />
        <MarqueeTrack items={ROW_2} reverse speed={40} />
      </div>
    </div>
  );
}
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid md:grid-cols-2 gap-12 items-end mb-20">
          ...
          <p className="font-body text-sm text-gray-500 leading-relaxed md:mb-2">
            Most tools bury you in setup, onboarding, and meeting links. We
            stripped all of that. You should be talking to your team in under 60
            seconds.
          </p>
        </div>
          ...
          {HOW_IT_WORKS.map((step, i) => (
            <div
              ...
              <div className="md:col-span-4">
                ...
                <h3 className="font-display font-bold text-gray-950 text-xl tracking-tight leading-snug">
                  {step.title}
                </h3>
              </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 divide-x divide-y divide-black/5 border border-black/5 rounded-2xl overflow-hidden shadow-sm">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="bg-white hover:bg-indigo-50/40 transition-all duration-200 p-8 group cursor-default">
      ...
      <div className="max-w-6xl mx-auto">
        <div className="relative rounded-3xl bg-gray-950 px-10 md:px-20 py-20 overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl">
            <p className="font-body text-xs font-semibold text-indigo-400/80 uppercase tracking-widest mb-6">
              The idea
            </p>
            <blockquote className="font-display font-extrabold text-white text-3xl md:text-4xl leading-tight tracking-tight mb-8">
              "The best remote conversations happen when they don't feel
              scheduled."
            </blockquote>
            <p className="font-body text-sm text-gray-400 leading-relaxed max-w-xl">
              Spatial proximity gives conversation back its context. You see
              who's nearby, who's in a cluster, who's available. No calendar. No
              link. Just presence.
            </p>
          </div>
        </div>
          ...
          {FAQS.map((faq, i) => (
            <div
              ...
              <button
                ...
                <span
                  className={[
                    ...
                    open === i
                      ? "border-indigo-300 bg-indigo-100 text-indigo-600 rotate-45"
                      : "border-gray-200 text-gray-400 group-hover:border-indigo-300 group-hover:text-indigo-500",
                  ].join(" ")}>
              ...
              <div
                ...
                <p className="font-body text-sm text-gray-500 leading-relaxed px-6 pb-5">
                  {faq.a}
                </p>
              </div>
            </div>
...
export default function Landing() {
  ...
  const handleCTA = (intent) => setModal(intent);
  ...
  // Called when map is selected in drawer
  const handleMapSelect = (mapId) => {
    ...
    socket.once("room-created", ({ roomId }) => {
      navigate(`/game/${roomId}`, {
        state: { username: pendingUsername, mapId, roomId },
      });
    });
  };
